...
class FirestoreService {
  ...
  Future getDonations() async {
    try {
      var donationDocumentSnapshot =
          await _donationsCollectionReference.getDocuments();
      if (donationDocumentSnapshot.documents.isNotEmpty) {
        return donationDocumentSnapshot.documents
            .map((snapshot) => Donations.fromMap(snapshot.data))
            .toList();
      }
    } catch (e) {
      if (e is PlatformException) return e.message;
